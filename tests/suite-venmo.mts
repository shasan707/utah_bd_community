...
t("email text offers venmo with the name", /Venmo instead\? Pay @evue007 \(Shamim Ahmmed\)/.test(onMail.text), true);
t("email leads with zelle", onMail.text.indexOf("Open Zelle") < onMail.text.indexOf("Venmo instead"), true);
t("email is a numbered checklist", /1\. Copy your code:\s+R-4K7M[\s\S]*2\. Open Zelle[\s\S]*3\. Send \$52\.00 to bpau\.pay@gmail\.com[\s\S]*4\. Paste R-4K7M in the memo/.test(onMail.text), true);
t("email says what Zelle will show", /Zelle will show the name Md Shamim Ahmmed\. That is us\./.test(onMail.text), true);
...
t("email says the code goes in the note", /paste R-4K7M in the memo or note/i.test(onMail.text), true);
t("text is a numbered checklist too", /1\) Copy the code 2\) Open Zelle or Venmo @evue007 3\) Send \$52\.00 to bpau\.pay@gmail\.com 4\) Paste R-4K7M in the memo\./.test(pendingSms(row, sv)), true);
