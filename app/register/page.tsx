...
const steps = [
  {
    n: "1",
    title: "Fill the form",
    text: "Tell us who is coming, plus any raffle draw coupons.",
  },
  {
    n: "2",
    title: "Send with",
...
export default async function RegisterPage() {
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-3xl px-5 pb-14 md:pb-16">
        <Reveal>
          <a
            href="https://www.facebook.com/BSAUofU"
            target="_blank"
            rel="noopener noreferrer"
            className="group flex flex-wrap items-center gap-x-3 gap-y-2 rounded-2xl border border-sand bg-white px-4 py-3 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-forest/40 hover:shadow-md hover:shadow-forest/10"
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-forest text-ivory transition-transform duration-300 group-hover:scale-110">
              <Icon name="facebook" className="h-4 w-4" />
            </span>
            <span className="min-w-0 flex-1 text-sm leading-snug text-forest-ink">
              <strong className="font-semibold">Are you a student?</strong>{" "}
              <span className="text-muted-ink">
                Student places are arranged by the student association, not this
                form.
              </span>
            </span>
            <span className="inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-forest">
              Message them
              <span
                aria-hidden="true"
